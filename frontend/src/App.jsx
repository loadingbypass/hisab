    ...
    <div style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
          ...
          <button type="button" onClick={() => showToast('Google login is not active yet', 'info')} style={{
            width: '100%', padding: '1rem', background: '#1c1c1e',
            color: 'white', border: '1px solid #2c2c2e', borderRadius: '14px', fontSize: '0.95rem', fontWeight: 600,
            cursor: 'pointer', marginTop: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', transition: 'background 0.2s'
          }}>
              ...
              return (
                <tr key={c.id}>
                  ...
                  <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                    {editingCashId === c.id ? (
                      <div>
                        <button className="btn-success" style={{ padding: '0.2rem 0.4rem', fontSize: '0.75rem', marginBottom: '0.2rem', width: '100%' }} onClick={() => handleUpdateCash(c.id)}>Save</button>
                        <button className="btn-danger" style={{ padding: '0.2rem 0.4rem', fontSize: '0.75rem', width: '100%' }} onClick={() => setEditingCashId(null)}>Cancel</button>
                      </div>
                    ) : (
                      <button className="btn-secondary" style={{ padding: '0.2rem 0.4rem', fontSize: '0.75rem', width: '100%' }} onClick={() => { setEditingCashId(c.id); setEditCashForm({ ami_pai: c.ami_pai, se_pay: c.se_pay }); }}>✏️ Edit</button>
                    )}
                  </td>
                </tr>
              )
